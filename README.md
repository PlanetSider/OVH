...
- 账户连接状态检查
- 支持按账户分离抢购队列与历史记录
- 正式保存 `alias`、`email`、`id` 三类账户标识

### 账户显示规则

系统当前对账户信息采用两套展示策略：

- 对话 / Bot 场景：优先显示 `alias`
  - 规则：`alias > email > id`
  - 适用场景：Telegram、飞书、机器人交互提示、账户切换文案

- 管理 / 调试 / 日志场景：显示完整账户标签
  - 规则：`alias | email | id`
  - 适用场景：账户管理接口、状态接口、关键日志、排障定位

说明：

- `alias` 适合作为日常操作时的人类可读主标签
- `email` 作为正式账户字段保存，便于多账户辨识
- `id` 作为系统稳定标识保留，不参与对话优先展示，但会保留在管理与日志上下文中
...
- 支持重装系统、切换启动模式等常见操作
- 页面顶部支持切换当前 API 账户
- 下方服务器列表仅展示当前 API 账户下的已购服务器
- 支持已购服务器资产自动刷新缓存，并可在“系统设置”中配置刷新间隔

## 多账户刷新规则

当前多账户场景下，不同功能采用不同的刷新与执行边界：

- 服务器列表自动刷新：仅使用“主刷新账号”执行
- 实时可用性自动刷新：仅使用“主刷新账号”执行
- 服务器监控：按账号独立运行
- 抢购队列：按账号独立执行
- VPS 补货通知：按账号独立订阅、独立线程、独立启停
- 服务器控制页已购服务器资产刷新：遍历所有 API 账户定期刷新

说明：

- “主刷新账号”可在“系统设置”中配置
- 主刷新账号只影响可售服务器列表与实时可用性两条链路
- 不影响当前页面选中的操作账号，也不影响服务器监控、抢购队列、VPS 监控等账号隔离能力
...
- 使用 Flask `test_client()` 验证主刷新账号配置行为
- 使用 Flask `test_client()` 验证 VPS 订阅/状态/启停按账号独立
- 使用 Flask `test_client()` 验证服务器别名在缓存读取路径中即时生效
- 使用 monkey patch 验证 `/api/server-control/list` 按 `X-OVH-Account` 返回不同账号的服务器列表
...
当前结论：SQLite 主存储方案已经能在本地真实初始化并通过核心 API 验证；主刷新账号、服务器控制账号隔离、VPS 按账号独立监控等关键行为也已完成本地验证。
...
- 服务器控制页默认优先读取已购服务器资产缓存；点击页面刷新按钮会触发强制实时刷新
